...
## 용량 관리 (무료 500MB 유지)
수집 작업이 끝날 때마다 `scripts/prune.py`가 DB 용량을 확인하고, 목표(기본 420MB)를 넘으면 자동으로 정리합니다.

**절대 지우지 않는 것** — 누군가 북마크·메모·읽음 표시한 논문, 열람 기록에 있는 논문, 댓글이 달린 논문, 추천에 올라간 논문, AI 요약이 있는 논문. 즉 북마크·히스토리·추천은 그대로 유지됩니다.

**정리 순서**
1. 1년보다 오래됐고 아무도 손대지 않았으며 요약도 없는 논문의 **초록만 비웁니다.** 행은 남아서 목록·검색·PubMed 링크가 계속 동작하고, 나중에 필요하면 같은 논문을 다시 수집하거나 검색창의 PubMed 구역에서 다시 가져오면 초록이 채워집니다. 용량의 대부분이 초록이라 보통 이 단계에서 끝납니다.
2. 그래도 목표를 못 맞추면 오래된 순서로 그 논문들을 **삭제**합니다.

**조정** — 저장소 Settings → Secrets and variables → Actions → **Variables** 탭에서 `PRUNE_TARGET_MB`(기본 420), `PRUNE_KEEP_DAYS`(기본 365)를 바꿀 수 있습니다. Pro 플랜으로 올렸다면 `PRUNE_TARGET_MB`를 크게 잡으세요.

현재 용량과 보호 중인 논문 수는 **Admin 탭 개요**에서 볼 수 있습니다.
