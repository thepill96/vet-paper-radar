...
create policy "members read active announcements" on public.announcements
  for select to authenticated using (active and public.is_approved() or public.is_admin());
...
-- 3. 요약 사용량 로그 ---------------------------------------------------------
create table if not exists public.summary_log (
  id            bigserial primary key,
  paper_id      uuid references public.papers(id) on delete set null,
  user_id       uuid references auth.users(id) on delete set null,   -- 자동 요약이면 null
...
create index if not exists summary_log_created_idx on public.summary_log (created_at desc);
alter table public.summary_log enable row level security;
-- 기록은 service role(스크립트·Edge Function)만 함. 관리자는 집계 RPC로만 조회.

-- 4. 관리자 집계 RPC ----------------------------------------------------------
create or replace function public.admin_user_stats()
returns table (id uuid, email text, display_name text, status text, is_admin boolean, created_at timestamptz,
...
language sql stable security definer as $$
  select p.id, p.email, p.display_name, p.status, p.is_admin, p.created_at,
    ...
    (select count(*) from public.user_papers u where u.user_id = p.id and u.is_read),
    (select count(*) from public.user_papers u where u.user_id = p.id and u.is_bookmarked),
    (select count(*) from public.user_papers u where u.user_id = p.id and coalesce(u.note,'') <> ''),
    ...
    greatest(
      ...
      (select max(updated_at) from public.user_papers u where u.user_id = p.id),
...
language sql stable security definer as $$
  select to_char(date_trunc('month', created_at), 'YYYY-MM'), source, count(*), sum(input_tokens), sum(output_tokens)
  from public.summary_log
  where public.is_admin()
